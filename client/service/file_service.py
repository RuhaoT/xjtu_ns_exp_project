...
from domain.file_model import ServerFileList, SingleFile, FileServerRequestAPI, FileServerRequestType, FileServerResponseAPI, FetchServerFileInterface, FileDownloadInterface, FileUploadInterface, FileDownloadResult, FileUploadResult
...
class FileService:
    ...
    def __init__(self, http_client: HttpClientSocket, local_file_backend: LocalFileBackend):
        self.http_client = http_client
        self.local_file_backend = local_file_backend
    
    def fetch_server_file_list(self, layer_request_interface: FetchServerFileInterface) -> ServerFileList:
        try:
            ...
            request.payload_bytes, request.content_length_before_encoding = encode_file_api_to_json(
                FileServerRequestAPI(
                    request_type=FileServerRequestType.LIST_FILES
                )
            )
            ...
            if response.vaild_response:
                if response.http_response.status_code == 200:
                    response_data = json.loads(response.http_response.payload_bytes.decode('utf-8'))
                    if 'request_success' in response_data and response_data['request_success']:
                    ...
                    return ServerFileList(
                        valid_list=False,
                        error_message="Failed to fetch file list from server." + error_message
                    )
            else:
                return ServerFileList(
                    valid_list=False,
                    error_message="Invalid response from server." + response.error_message
                )
        except Exception as e:
            return ServerFileList(
                valid_list=False,
    ...
    def download_file_batch(self, layer_request_interface: FileDownloadInterface) -> FileDownloadResult:
        ...
        try:
            ...
            current_server_file_list = self.fetch_server_file_list(fetch_file_list_request)
            if not current_server_file_list.valid_list:
                raise RuntimeError("Error fetching server file list before download: " + current_server_file_list.error_message)
            ...
            for file_name in file_name_list:
                if not any(file.file_name == file_name for file in current_server_file_list.file_list):
                    raise RuntimeError(f"File '{file_name}' not found on server.")
                else:
                    request_download_file_info_list.append(
                        next(file for file in current_server_file_list.file_list if file.file_name == file_name)
                    )
            ...
            for file_info in request_download_file_info_list:
                local_file_path = layer_request_interface.setting.local_file_dir + file_info.file_name
                if os.path.exists(local_file_path):
                    local_file_hash = self.local_file_backend.get_file_hash(local_file_path)
                    if local_file_hash == file_info.file_hash:
                        # skip download
                        continue
                    else:
                        raise RuntimeError(f"Local file '{file_info.file_name}' exists but has hash mismatch.")
                else:
            ...
            http_request.payload_bytes, http_request.content_length_before_encoding = encode_file_api_to_json(
                ...
                if response.http_response.status_code == 200:
                    ...
                    if 'request_success' in response_data and response_data['request_success']:
                        ...
                        for file_info in actual_download_file_info_list:
                            ...
                            downloaded_file = next((file for file in downloaded_file_list if file['file_name'] == file_info.file_name), None)
                            if downloaded_file is None:
                                raise RuntimeError(f"Downloaded file '{file_info.file_name}' not found in response.")
                            ...
                            file_bytes_base64 = downloaded_file['file_data'].encode('ascii')
                            file_bytes = base64.b64decode(file_bytes_base64)
                            
                            self.local_file_backend.save_file(local_file_path, file_bytes)
                            actual_download_file_name_list.append(file_info.file_name)
                        return FileDownloadResult(download_success=True, downloaded_file_name_list=actual_download_file_name_list)
                    else:
                        return FileDownloadResult(
                            download_success=False,
                    ...
                    return FileDownloadResult(
                        download_success=False,
                        error_message="Failed to download files from server." + error_message
                    )
            else:
                return FileDownloadResult(
                    download_success=False,
    ...
    def upload_file_batch(self, layer_request_interface: FileUploadInterface):
        ...
        try:
            ...
            current_server_file_list = self.fetch_server_file_list(fetch_file_list_request)
            if not current_server_file_list.valid_list:
                raise RuntimeError("Error fetching server file list before upload: " + current_server_file_list.error_message)
            ...
            for file_name in file_name_list:
                local_file_path = layer_request_interface.setting.upload_file_dir + file_name
                if not os.path.exists(local_file_path):
                    raise RuntimeError(f"File '{file_name}' not found on local.")
                else:
                    # base64 encode the file to ascii string
                    file_bytes = self.local_file_backend.load_file(local_file_path)
                    ...
                    request_upload_file_info_list.append(
                        SingleFile(
                            file_name=file_name,
                            file_hash=self.local_file_backend.get_file_hash(local_file_path),
            ...
            for file_info in request_upload_file_info_list:
                if any(file.file_name == file_info.file_name for file in current_server_file_list.file_list):
                    server_file_info = next(file for file in current_server_file_list.file_list if file.file_name == file_info.file_name)
                    if server_file_info.file_hash == file_info.file_hash:
                        # skip upload
                        continue
                    else:
                        raise RuntimeError(f"Server file '{file_info.file_name}' exists but has hash mismatch.")
                else:
                    # add to upload list
            ...
            if response.vaild_response:
                if response.http_response.status_code == 200:
                    ...
                    if 'request_success' in response_data and response_data['request_success']:
                        ...
                        return FileUploadResult(upload_success=True, uploaded_file_name_list=uploaded_file_name_list)
                    else:
                        return FileUploadResult(
                            upload_success=False,
                    ...
                    return FileUploadResult(
                        upload_success=False,
                        error_message="Failed to upload files to server." + error_message
                    )
            else:
                return FileUploadResult(
                    upload_success=False,
