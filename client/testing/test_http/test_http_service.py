...
def fetch_http_request_data(yaml_data: dict):
    ...
    return HTTPLayerInterfaceRequest(
        ...
        maintain_session_during_redirects=yaml_data[
            "maintain_session_during_redirects"
        ],
    )
...
def test_http_client_socket_form_login():
    ...
    http_request.payload_bytes, http_request.content_length_before_encoding = (
        load_login_test_data(test_data)
    )
...
def test_http_client_socket_form_login_send_encoding():
    ...
    encoding_combinations = list(
        itertools.product(transfer_encodings, content_encodings)
    )
    ...
    for encoding in encoding_combinations:
        ...
        assert isinstance(response, HTTPLayerInterfaceResponse)
        assert (
            response.vaild_response == test_data["expected_response"]["valid_response"]
        )
        assert (
            response.http_response.status_code
            == test_data["expected_response"]["status_code"]
        ...
        assert (
            response.http_response.location
            == test_data["expected_response"]["location"]
        )
        assert response.http_response.set_cookie != None
        assert response.error_message == test_data["expected_response"]["error_message"]
